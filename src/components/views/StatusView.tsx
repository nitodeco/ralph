...
function StatusHeader({ version }: { version: string }): React.ReactElement {
	return (
...
function StatusFooter(): React.ReactElement {
	return (
		<Box paddingX={1}>
...
export function StatusView({ version, onClose }: StatusViewProps): React.ReactElement {
	...
	return (
		<ResponsiveLayout
			header={<StatusHeader version={version} />}
			content={
				<ScrollableContent>
							...
							<>
								<Box flexDirection="column" marginTop={1}>
									...
									<Box flexDirection="column" paddingLeft={2}>
										...
										<Text>
											Started:{" "}
											<Text color="cyan">{new Date(session.startTime).toLocaleString()}</Text>
										</Text>
										<Text>
											Last Update:{" "}
