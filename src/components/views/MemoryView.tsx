...
export const MemoryView: React.FC<MemoryViewProps> = ({ version, onClose }) => {
  ...
  const tabs: { key: MemoryTab; label: string; count: number }[] = [
    { count: memory.lessonsLearned.length, key: "lessons", label: "Lessons" },
    { count: memory.successfulPatterns.length, key: "patterns", label: "Patterns" },
    { count: memory.failedApproaches.length, key: "failed", label: "Failed" },
    { count: Object.keys(memory.taskNotes).length, key: "notes", label: "Notes" },
  ];
  ...
  useInput((input, key) => {
    if (viewMode === "confirm-clear") {
      ...
      if (key.return) {
        ...
        setMessage({ text: "Session memory cleared", type: "success" });
        setViewMode("view");
    ...
    if (input === "c") {
      ...
      if (
        ...
        setMessage({ text: "Session memory is already empty", type: "error" });
        setTimeout(() => setMessage(null), MESSAGE_DISMISS_TIMEOUT_MS);
      } else {
