...
export function GitHubSetupView({ version, onClose }: GitHubSetupViewProps): React.ReactElement {
  ...
  const { gitProvider } = globalConfig;
  ...
  useInput((input, key) => {
    if (state.step === "menu" && (key.escape || input === "q")) {
      ...
      setState((prev) => ({ ...prev, message: null, step: "menu" }));
    } else if (state.step === "oauth" && (key.escape || input === "q")) {
      isPollingRef.current = false;
      setState((prev) => ({
        ...prev,
        codeCopied: false,
        message: { text: "OAuth flow cancelled", type: "info" },
        oauthDeviceCode: null,
        step: "menu",
      }));
    ...
    const pollForToken = async () => {
      ...
      while (Date.now() < expiresAt && !isCancelled) {
        ...
        if (result.status === "success") {
          ...
          setState((prev) => ({
            ...prev,
            message: { text: "Successfully authenticated with GitHub!", type: "success" },
            step: "complete",
          }));
        ...
        if (result.status === "error") {
          ...
          setState((prev) => ({
            ...prev,
            message: { text: `Authentication failed: ${result.error}`, type: "error" },
            oauthDeviceCode: null,
            step: "menu",
          }));
      ...
      if (!isCancelled) {
        ...
        setState((prev) => ({
          ...prev,
          message: { text: "Authorization timed out. Please try again.", type: "error" },
          oauthDeviceCode: null,
          step: "menu",
        }));
      }
  ...
  const handleMenuSelect = (item: { value: string }) => {
    match(item.value)
      ...
      .with("token", () => {
        setState((prev) => ({ ...prev, message: null, step: "token" }));
      })
      .with("auto_pr", () => {
        setState((prev) => ({ ...prev, message: null, step: "auto_pr" }));
      })
      .with("pr_draft", () => {
        setState((prev) => ({ ...prev, message: null, step: "pr_draft" }));
      })
      .with("clear", () => {
        const updatedConfig = {
          ...globalConfig,
  ...
  const handleTokenSubmit = () => {
    ...
    if (!trimmedToken) {
      setState((prev) => ({
        ...prev,
        message: { text: "Token cannot be empty", type: "error" },
      }));
