...
export const AnalyzeView: React.FC<AnalyzeViewProps> = ({ version, onClose }) => {
  ...
  useInput((input, key) => {
    ...
    if (input === "c") {
      clearFailureHistory();
      setMessage({ text: "Failure history cleared", type: "success" });
    ...
    if (suggestedGuardrails.length > 0) {
      ...
      if (key.return && suggestedGuardrails[selectedSuggestionIndex]) {
        ...
        try {
          ...
          setMessage({ text: `Added guardrail: "${guardrail.instruction}"`, type: "success" });
        } catch {
          setMessage({ text: "Failed to add guardrail", type: "error" });
        }
