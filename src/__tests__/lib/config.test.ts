...
describe("validateConfig", () => {
  ...
  test("validates agentTimeoutMs allows zero (disabled)", () => {
    ...
    const validPositive = validateConfig({ agent: "cursor", agentTimeoutMs: 60_000 });
  ...
  test("validates memory config", () => {
    const validMemory = validateConfig({
      ...
      memory: {
        ...
        maxOutputBufferBytes: 1_048_576,
...
describe("applyDefaults", () => {
  ...
  test("merges memory config with defaults", () => {
    const result = applyDefaults({
      memory: {
        maxOutputBufferBytes: 2_000_000,
      },
    });

    expect(result.memory?.maxOutputBufferBytes).toBe(2_000_000);
  });

  test("preserves optional fields when provided", () => {
    const result = applyDefaults({
      lastUpdateCheck: 1_234_567_890,
      skipVersion: "1.0.0",
    });

    expect(result.lastUpdateCheck).toBe(1_234_567_890);
