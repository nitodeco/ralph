...
describe("formatVerificationResult", () => {
  ...
  test("formats multiple checks", () => {
    const result: VerificationResult = {
      checks: [
        { durationMs: 100, name: "build", output: "", passed: true },
        { durationMs: 200, name: "test", output: "Failed", passed: false },
      ],
      failedChecks: ["test"],
      passed: false,
      totalDurationMs: 300,
  ...
  test("includes duration in summary", () => {
    const result: VerificationResult = {
      checks: [{ durationMs: 150, name: "build", output: "", passed: true }],
      failedChecks: [],
      passed: true,
      totalDurationMs: 150,
    };
...
describe("generateVerificationRetryContext", () => {
  ...
  test("returns empty string when no failed checks", () => {
    const result: VerificationResult = {
      checks: [{ durationMs: 100, name: "build", output: "", passed: true }],
      failedChecks: [],
      passed: true,
      totalDurationMs: 100,
    };
  ...
  test("includes multiple failed checks", () => {
    const result: VerificationResult = {
      checks: [
        { durationMs: 100, name: "build", output: "Build failed", passed: false },
        { durationMs: 200, name: "test", output: "Tests failed", passed: false },
      ],
