...
class Logger {
  ...
  logAgentRetry(retryCount: number, maxRetries: number, delayMs: number): void {
    this.warn("Agent retry scheduled", { delayMs, maxRetries, retryCount });
  }
  ...
  logIterationComplete(
    ...
    this.info("Iteration completed", { isProjectComplete, iteration, totalIterations });
  }

  logIterationFailure(iteration: number, error: string): void {
    this.error("Iteration failed", { error, iteration });
  }

  logSessionStart(totalIterations: number, taskIndex: number): void {
    this.info("Session started", { taskIndex, totalIterations });
  }
  ...
  logSessionResume(
    ...
    this.info("Session resumed", { currentIteration, elapsedTimeSeconds, totalIterations });
  }
