...
export type Linter = "eslint" | "biome" | "oxlint" | "unknown";
export type Formatter = "prettier" | "biome" | "oxfmt" | "unknown";
...
function detectLinter(projectPath: string, packageJson: PackageJson | null): Linter {
  ...
  if (
    existsSync(join(projectPath, ".oxlintrc.json")) ||
    existsSync(join(projectPath, "oxlint.config.ts"))
  ) {
    return "oxlint";
  }
...
function detectFormatter(projectPath: string, packageJson: PackageJson | null): Formatter {
  ...
  if (
    existsSync(join(projectPath, ".oxfmtrc.json")) ||
    existsSync(join(projectPath, ".oxfmtrc.jsonc"))
  ) {
    return "oxfmt";
  }
...
function detectScripts(packageJson: PackageJson | null): CodebaseAnalysis["detectedScripts"] {
  ...
  const scriptPatterns = {
    ...
    format: ["format", "prettier", "biome format", "oxfmt"],
    lint: ["lint", "eslint", "biome check", "oxlint"],
    test: ["test", "spec"],
    ...
    const lintCommand = analysis.detectedScripts.lint
      ...
      : analysis.linter === "biome"
        ...
        : analysis.linter === "oxlint"
          ? `${packageManagerCommand} run oxlint`
  ...
  if (analysis.formatter !== "unknown" && analysis.formatter !== analysis.linter) {
    const formatCommand = analysis.detectedScripts.format
      ...
      : analysis.formatter === "biome"
        ...
        : analysis.formatter === "oxfmt"
          ? `${packageManagerCommand} run oxfmt`
