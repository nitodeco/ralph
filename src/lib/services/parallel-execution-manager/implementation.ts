...
export function createParallelExecutionManager(
  ...
  const parallelTaskResults = new Map<string, ParallelTaskResult>();
  ...
  function initialize(
    ...
    if (!validationResult.isValid) {
      ...
      return { error: `Invalid task dependencies: ${errorMessages}`, isValid: false };
    }
  ...
  function startNextGroup(): StartGroupResult {
    ...
    if (currentGroupIndex >= parallelExecutionGroups.length) {
      ...
      return { groupIndex: -1, started: false, tasks: [] };
    }
    ...
    return { groupIndex: currentGroupIndex, started: true, tasks: tasksToExecute };
  }
  ...
  function recordTaskComplete(
    ...
    if (!currentParallelGroup) {
      ...
      return { allSucceeded: false, groupComplete: true };
    }
