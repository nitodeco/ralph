...
export function createBranchModeManager(): BranchModeManager {
  ...
  async function completeTaskBranch(prd: Prd | null): Promise<CompleteTaskBranchResult> {
    ...
    if (shouldReturn && baseBranch) {
      ...
      if (returnResult.status === "error") {
        ...
        return { error: returnResult.error, success: false };
      }
  ...
  async function createPullRequestForBranch(
    ...
    if (!branchModeEnabled || !baseBranch) {
      return { error: "Branch mode not enabled or no base branch", success: false };
    }
    ...
    if (!remoteUrl) {
      ...
      return { error: "No remote URL configured", success: false };
    }
    ...
    if (remoteInfo.provider === "none") {
      ...
      return { error: `Unknown git provider for remote: ${remoteUrl}`, success: false };
    }
    ...
    if (!provider) {
      ...
      return { error: `${remoteInfo.provider} provider not configured`, success: false };
    }
    ...
    try {
      ...
      return { prUrl: result.data.url, success: true };
    } catch (error) {
