...
- 0be7826: Fix shutdown and signal handling races by making shutdown entry atomic, preventing duplicate signal handler registration, and hardening cleanup paths so session persistence, UI unmount, and service bootstrap failures do not leave the process in a partial state.
- 9bc3916: Fix subprocess lifecycle handling in `AgentProcessManager` so abort state survives process teardown and force-kill escalation runs reliably when child processes do not exit after `SIGTERM`.
